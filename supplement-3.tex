...
Such changes leave the profiles unchanged since both the units of the vertices $\vec{x}_i$ and
of the displacement uncertainty $\sigma$ are rescaled (see
equation~\ref{eq:parameter-estimation-log-likelihood}).
Furthermore, values of the lineage-aware image-space metric
(equation~\ref{eq:Lineage-aware-image-space-metric}) are also scale-invariant since the image
...
\begin{table}[H]
    \begin{tabularx}{\textwidth}{l r@{} @{\hspace{0.3em}}l X}
        ...
        $N_v$ & $8$ && Number of vertices used for the discretized rod\\
        $\sigma_x$ & $0.15$&$\unit{\micro\metre}$ & Displacement uncertainty used to calculate the
        profile-likelihood\\
        ...
        \bottomrule
    \end{tabularx}
    \caption[Estimated Constants used in the Numerical Simulation]{
        Manually estimated constants which are used within the numerical simulation.
    }
...
(not normalized) of the various optimizations.
Figure (A) shows that all optimization routines converge, with varying properties.
The morse-type models achieve a lower (better) cost while both mie-type models remain at higher cost
values.
In addition, the reduced models where $\lambda=\SI{1}{\per\minute}$ converge faster.
These findings further corroborate the results presented in
Subsection~\ref{sec:application-1-without-division}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm-fit-divide-progression.pdf}
    \caption[Cost Value Evolution of all Optimization Routines]{
        Optimization progression of the distinct model choices analyzed in
        Section~\ref{section:application-2}.
        All cost functions exhibit rapid decay before a final optimum is reached.
...
\begin{figure}[H]
    \includegraphics[width=\textwidth]{figures/crm_divide/profiles-2.pdf}
    \caption[Remaining Parameter Profiles ofr Model including Division Events]{
        ...
        The profiles show that the parameters can be informed by the provided data.
...
To assess the accuracy of the estimated parameters in relation to our model, we can examine
...
Our system is inherently non-linear, containing exponential growth and complex interaction forces.
This means that any parameter choice which is not optimal can lead to large deviations with
increasing time.
For a Lipschitz-continuous system, this deviation can be bound by an exponential envelope of
...
$p_p$ is used.
When increasing the growth rates of all cells by $20\%$ (blue curves), the non-linear
nature of our system becomes apparent.
...
\begin{figure}[H]
    ...
    \caption[Contributions to the Lineage-Aware Image-Space Metric.]{
