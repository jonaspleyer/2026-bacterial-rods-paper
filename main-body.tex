...
%
%     \item Literature Gap 1: No \acp{abm} support flexible rod-shaped bacteria
...
%     \item Why matter 3: Bacteria especially rod-shaped are fundamental. Understanding them is
%         important.
%
...
spirals and helices.
Rod-shaped bacteria form an important subclass that includes some of the most studied model
organisms such as \ac{ecoli} or \ac{bsubtilis}.
Their filamentous structure imposes mechanical~\cite{Daniel2003} and physiological
...
Mechanical flexibility, however, is a feature that is absent in most \acp{abm} for rod-shaped
bacteria.
...
shapes~\cite{Rudge2012,Kang2014,Breitwieser2021,Gorochowski2012}, typically modeling cells as rigid
spherocylinders interacting through steric repulsion~\cite{Rudge2012,Winkle2017}.
While such models can couple mechanical stress to cellular growth~\cite{Winkle2017}, we are not
aware of a framework that captures the elastic bending of flexible filamentous bacteria or provides
a route to quantitative, single-cell parameter estimation from image data.
Although \acp{abm} appear to be ideal for modeling individual rod-shaped bacteria, this potential
...
Approaches for analyzing and calibrating \acp{abm} have been proposed~\cite{An2016,Thiele2014},
and individual models have been fitted to experimental data~\cite{Dancik2010,Lima2021}, but these
methods are not widely applied and are generally not used to explicitly capture any possible
heterogeneity within the cellular population.

However, this can leave important questions unanswered:
which combination of parameters drives the observed phenomena?
...
parameters or how they are constrained. Second, models which are not properly calibrated,
i.e., by a data-based approach or via
domain-specific knowledge, cannot guarantee to accurately represent the underlying biological
reality.
Finally, rod-shaped bacteria are fundamental organisms which can be seen as a precursor to
...
and provide a framework that links mechanistic modeling of rod-shaped bacteria with quantitative
analysis methods (Figure~\ref{fig:flowchart-project-structure}).
First, we construct a novel mechanical model for flexible, rod-shaped bacteria that captures growth,
division, elastic deformations of the rod and mechanical interactions between cells.
Second, we develop computational methods that use segmented microscopic images to
...
representation of bacteria as discretized vertices, others, such as the lineage-aware image-space
metric, operate purely on segmentation masks, making them fully model-agnostic.
This enables researchers to freely reuse our developed methods across a wide range of models for
rod-shaped organisms.

Our mechanical model integrates into our previously developed agent-based modeling framework
\texttt{cellular\_raza}~\cite{Pleyer2025}, which makes no particular assumptions about the cellular
representation or the nature of the modeled processes and thus lets researchers adapt the underlying
cellular model to their needs.
We apply the resulting framework to two case studies of increasing complexity, each probing
different aspects of the model.
For a single bent bacterium, we find the full mechanical model to be non-identifiable and use
profile likelihoods to guide its reduction to an identifiable one.
For a growing colony, we recover growth rates and division lengths of individual cells and their
descendants from routine time-lapse microscopy.
Parameter estimation for \acp{abm} is thus feasible even when the model resolves individual,
flexible rod-shaped cells and accounts for cellular heterogeneity.
%###################################################################################################
% \section{Overview}
% This work consists of many subcomponents which can be combined to achieve various tasks.
...
% In the future, we are planning to extend the rendering capabilities of our approach and
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/concept/overview.tikz}
    \caption[Modular framework for parameter estimation of rod-shaped bacteria.]{
        Modular framework for parameter estimation of rod-shaped bacteria.
        ...
        flexible, rod-shaped bacteria (middle), and data extraction routines (bottom).
        ...
        Colors indicate the component type; core scientific drivers (orange) and auxiliary
        methods (yellow).
    }
...
The mechanical behavior of rod-shaped bacteria is governed by many effects.
...
its parameters. We thus use only a minimal set of assumptions.
...
We therefore need to account for this phenomenon as well.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{c l X}
        ...
        \bottomrule
    \end{tabularx}
    \caption[Simulation Aspects of Elongated Bacteria Computational Model]{
...
To model the spatial mechanics of rod-shaped bacteria \cite{Billaudeau2017} mathematically, we represent them as a
...
This representation alone is sufficient to build the presented visualization and data-extraction
pipelines (see Figure~\ref{fig:flowchart-project-structure}), while our computational model uses it
to define the dynamics of the vertices $\{\vec{x}_i\}$.
...
\begin{figure}[H]
    ...
    \caption[Schematics for Cellular Mechanics and Cell-Cell Interactions.]{
        (A) Example of a bacterial rod represented by a collection of vertices which are
        connected by springs.
        The angles $\alpha_i$ are used to model the rigidity of the rod.
        ...
        upper bound $\beta=2.5\,V_0$ and cutoff $\zeta=3.5\,(R_1+R_2)$ (red line).
...
Cells which are under tension over longer time scales will therefore exhibit asymmetric,
heterogeneous intracellular growth of the cell wall which results in plastic
deformations~\cite{Ursell2014}.
However, without any external interference, the bacteria will grow into straight rod-like
shapes~\cite{Wang2010}.
Our work focuses on smaller time scales where no plastic deformations are involved but short-term
...
Figure~\ref{fig:model-bacterium} (A) shows a bacterial rod represented by $6$ vertices.
...
\end{equation}
The spring constant $\gamma$ determines the elasticity with respect to elongation of the cell.
...
\end{equation}
where $\eta$ is the rigidity of the rod (see Figure~\ref{fig:model-bacterium}).
...
The equations have been rescaled by the mass, thus effectively
...
system.
The interactions can be phenomenologically described by simple, interpretable potentials with a low
...
However, the latter has only three parameters, which may be advantageous in situations of sparse
data.
...
It is given by:
\begin{align}
    V(r) &= V_0\,c_{nm}\left[ \left(\frac{r_0}{r}\right)^n -
        \left(\frac{r_0}{r}\right)^m\right]\\
    c_{nm} &= \frac{n}{n-m}\left(\frac{n}{m}\right)^{\frac{n}{n-m}}
\end{align}
The minimum of the potential is given by $-V_0$ and the thickness of our rods is encoded in the
characteristic distance $r_0 = (m/n)^{1/(n-m)}(R_1+R_2)$, where $R_1,R_2$ are the radii of the two
interacting rod-like bacteria.
...
The overall potential strength $V_0$, the sum of the radii of the particles $R_1+R_2$ and the
potential stiffness $\omega$ which controls the rate of change as one travels along the radial axis.
We also assign a spatial cutoff $\zeta$ to the range of the interaction potential.
...
We first calculate the new spring lengths of the two resulting rods using the properties of the
mother cell:
\begin{equation}
...
event.
We subdivide the polygon given by the vertices of the mother cell in $2n+1$ segments consisting of
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c l c c}
        ...
        \bottomrule
    \end{tabular}
    \caption[List of all Parameters and Variables]{
...
\texttt{cellular\_raza} integrates seamlessly into Python code bases, enabling users to call
...
comparison algorithm (see Subsection~\ref{subsec:image-space-metric}).

%###################################################################################################
\section{Methodology}
\label{sec:methodology}
This section introduces methods that can be combined to estimate the parameters of our
computational model which are covered by the first 3 subsections.
The final subsection gives insights into the challenge of estimating heterogenous parameters.
...
Finally, we provide two distinct comparison algorithms which can be used to compare numerically
obtained results with experimental data.
We test and benchmark the constructed algorithms within the respective sections.
...
class of mechanical models that can capture different cellular behaviors.
...
\texttt{PyVista}~\cite{Sullivan2019}, which provides high-level wrappers around the popular
VTK library~\cite{vtkBook}.
...
adjacent vertices $\vec{x}_i,\vec{x}_{i+1}$ with a cylinder of radius $R$ that is positioned
...
Finally, we apply a parallel projection along the z-axis to obtain an image solely comprised
of solid colors.
...
\begin{figure}[H]
    ...
    \caption[Schematic visualizing the Imaging Pipeline]{
...
This section explains how to transform a segmented microscopic image into a collection of vertices
(see Subsection~\ref{subsection:mechanical-model-mechanics}) that can be used
to initialize agents and to perform parameter estimation.
A wide range of tools exists for image segmentation~\cite{Stringer2020,Cutler2022,Hardo2022}.
Figure~\ref{fig:position-extraction-algorithm} (A) shows a typical microscopic image.
Our algorithm requires that images be prepared using a pixel-based (in-image) encoding scheme, as
we explained in Subsection~\ref{subsection:visualization}.

% Having defined the input type of our algorithm, we proceed with the following 5 steps:
\begin{algorithm}
    \caption{The Position Extraction Algorithm}
    \label{algo:extraction-algorithm}
    \begin{algorithmic}[1]
        \Function{Extraction}{\texttt{mask}, \texttt{n\_vertices}}
        \State $\texttt{positions} \gets [\hspace{6pt}]$
        \Comment{Initialize empty array}
...
bacteria.
The inputs required are a segmented image and the desired number of vertices, $n$.

Figure~\ref{fig:position-extraction-algorithm} (A-C) visualizes this progression.
In the first step, the image is split into a collection of instance-level cell masks that only
...
To sort the points, we start with one of the endpoints and determine the next neighbor and
...
algorithm. By comparing the extracted positions with the known ground truth, we obtain
...
To quantitatively analyze this disparity we use both data to calculate the average rod
...
\begin{figure}[H]
    ...
    \caption[Detailed Analysis of the Position Extraction Algorithm]{
        ...
        exclude outliers.
        (L) Mean (curve) and variance (filled space) of fitted normal
...
To effectively estimate the parameters of our model, we need to set up a cost function
which can be minimized.
As explained in Section~\ref{sec:methodology}, parameters can be specified as global, which are
identical across all cells, or as individual, which allow us to capture cell-to-cell variability.
...
We first present a direct method which is applicable in scenarios involving no division events.
...
The previous sections have laid out how the positional information of cells can be obtained from
provided segmentation masks.
Given $N_c$ cells with $N_v$ vertices in $d$ dimensions at $N_t$ time points, the total information
can be stored in an array of shape $(N_c,N_v,d,N_t)$.
We denote the array of experimental data by $\tilde{\nu}_{ijkl}$ and the model output depending on
parameters $\theta$ by $\nu_{ijkl}(\theta)$.
However, these arrays will only retain their shape if no cells divide during the observed interval,
which limits the applicability of this approach (see Subsection~\ref{subsec:image-space-metric}).
Comparing the results of a simulation $\nu$ with experimental values $\tilde{\nu}$ is thus only a
matter of comparing these arrays via a suitable cost function~\cite{Wang2020} under a given error
model.
...
is given by
\begin{equation}
    L(\tilde{\nu}|\nu(\theta)) = \sum\limits_{ijkl}\left(\left(\frac{\nu_{ijkl}(\theta)-\tilde{\nu}_{ijkl}}{\sigma_{ijkl}}\right)^2
    + 2 \log(\sigma_{ijkl})
    +\text{const}\right)
...
\begin{equation}
    L(\tilde{\nu}|\nu(\theta))
        = \frac{L_\text{LS}(\tilde{\nu}|\nu(\theta))}{\sigma^2} + \text{const}.
\end{equation}
It is important to pick a value for $\sigma$ which presents an upper bound of the real uncertainty.
The confidence interval of the optimized parameters can be obtained via the
\textit{profile likelihood} method \cite{Kreutz2013}.
...
Ideally, a model with fully optimized parameters would not contain any time intervals of
non-matching bacterial configurations and would strive to reduce such mismatches.
However, these scenarios can manifest, especially during optimization and possibly even in the final
optimized result.

In addition to these technical challenges, there are practical ones.
Within a short period of time after the division event, the two resulting daughter cells will occupy
the same space as the mother cell.
However, the exact time of the fission event cannot be determined accurately, leading to uncertainty
regarding when the lives of the daughter cells begin and when the life of the mother cell ends.
These uncertainties are also a common problem for segmentation methods and further highlight the
importance of a purely image-based comparison scheme.

Consequently, the direct metric cannot be simply reused on confined intervals which
do not include division events: It does not account for the inaccuracy of the division timing.
...
Our proposed image-based method circumvents this fundamental limitation.
...
non-matching regions. Therefore, it is first necessary to define a criterion for this.

We can use the previously introduced visualization methods (see
Section~\ref{subsection:visualization}) methods to obtain cell-masks from the numerical results of
the \ac{abm} simulation.
Since we use an in-image encoding scheme, each pixel has a unique color value which is directly
...
It can occur, e.g., in dense areas, containing multiple cellular agents with low repulsive
...
This value should be chosen smaller than the default penalty $p=1$ for non-matching color
values to weaken the assigned penalty due to parental relationship.

To compare simulation results with data, we denote $I=I(\theta)$ to be the model output which
implicitly depends on the chosen parameters.
We define the total cost $L(\theta|I,\tilde{I})$ between two images $I$ and $\tilde{I}$ as the sum
of all individual penalties
\begin{align}
    L(\theta|I,\tilde{I}) &= \sum\limits_{ij} p(I_{ij}, \tilde{I}_{ij})\\
...
relationship.
By this definition, the parameters of the model are incorporated via $I=I(\theta)$.
In principle it is also feasible to compare one model output with another which would require us to
...
At a higher level, this means that our method is agnostic to the underlying cellular representation.
Thus, other models that potentially use very different cellular representations but can render
segmented images can reuse our method.
...
expected behavior.
We use our previously defined computational model (see \nameref{sec:computational-model}), to
...
\begin{figure}[H]
    ...
    \caption[Comparison of Fitting Methods with and without Parental Penalty]{
        (A,B) Sequential simulation masks before (A) and after (B) division events.
...
In this section we apply the above developed methods and models to two case studies of increasing
complexity and perform parameter estimations for each.
First, we fit our model to data of elastically bending bacteria published by Amir et.
al~\cite{Amir2014,Amir2014_2}.
This case study focuses on the mechanics of the flexible rod and excludes any complex cell-cell
interactions.
...
(see Section~\ref{section:application-2}), which we examine in two scenarios.
In the first scenario, we consider a short time interval that does not contain division events,
enabling us to employ the direct position-space metric (see Subsection~\ref{subsec:direct-metric}).
In the second scenario, we demonstrate the feasibility of our approach for multiple division
events by applying the lineage-aware image-space metric (see
Subsection~\ref{subsec:image-space-metric}) to a time series containing such events.

% --------------------------------------------------------------------------------------------------
\subsection{Application 1: Elastic deformations of \acl{ecoli}}
\label{sec:application-1-amir}
To demonstrate that our model can be applied to single cells and to showcase the usefulness
of the comparison techniques that we have previously developed, we apply our computational
model directly to the data provided by Amir et al.'s experiments~\cite{Amir2014,Amir2014_2}
...
and focuses on the behavior of a single cell.
...
is the total size of the chamber along the y-axis which is hidden in
...
\begin{align}
    \vec{G}_i &= C
    \underbrace{|\vec{x}_i - \vec{x}_{i+1}|\sin(\varphi_i)}_\text{proj. rod segment length}
...
Similarly to Stokes' Law~\cite{Stokes1856}, it acts along the axis of the fluid flow (x-axis).
Here $\varphi_i$ is the angle between the rod edge $\vec{x}_{i+1}-\vec{x}_i$ and the x-axis.
...
$\SI{7}{\second}$.
Figure~\ref{fig:amir-bending-simulation} (B) displays this intermediate state.
The second phase involves disabling the force, allowing the cell to return to its original shape (C)
...
We are thus left with 5 parameters: growth rate $\mu$, rigidity of the rod $\eta$, spring tension
$\gamma$ and damping constant $\lambda$ (see Table~\ref{table:list-of-parameters}), together with
the magnitude $C$ of the drag force introduced above.
Figure~\ref{fig:amir-bending-simulation} (D) shows a snapshot of the initial (upright, light blue)
...
(E-I, bright blue curves).
It can be seen that the optimized parameters are at the minimum of the
\textit{profile-likelihood}.
However, \textit{profile-likelihoods} shown in Figure~\ref{fig:amir-bending-simulation} (E-I) are
multimodal, e.g., the corresponding parameters are not unique.
...
It should be noted that the available images only contain snapshots of a fully bent or a fully
relaxed rod, with no intermediate steps in between.
The data therefore constrain the damping constant $\lambda$ only through the durations of the two
phases and not through the transient itself.
This is reflected in the shape of the corresponding profile in
Figure~\ref{fig:amir-bending-simulation} (I), which is flat towards zero but rises steeply for
larger values.
The damping constant is therefore practically non-identifiable from below while being bounded from
above; resolving it from below would require time-resolved information on the elastic relaxation,
which is not available for this experiment.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm_amir/profiles.pdf}
    \caption[\textit{Likelihood Profiles} of the Bending Bacteria Experiment by Amir et al.]{
        ...
        (E-I) Profile-likelihoods for the estimated parameters.
        Bands of blue and yellow indicate confidence intervals of $68\%,90\%$ and $95\%$.
        Bright blue curves represent the full model, dark blue a reduced model without damping
        $\lambda=0$ and orange curves when fixing the spring tension parameter
        $\gamma=\gamma_\text{opt}$, where $\gamma_\text{opt}$ is the second (higher) minimum of the
        spring tension profile (H) of the reduced model with $\lambda=0$.
        With these reductions, every parameter is identifiable; note that the confidence intervals
        of the reduced models are conditional on the parameters held fixed.
    }
...
Our dataset~\cite{Trude1982} consists of a time-series of phase-contrast images (see
...
previously described pipeline (Supporting Information,
\nameref{sec:supplement-extraction-algorithm}).
...
For our first scenario, we selected 7 individual images spanning over an interval of
...
We estimate the parameters of this system via the direct position-space metric
(see Section~\ref{subsec:direct-metric}).

In the second scenario, we extended the interval to $\SI{85}{\minute}$ with 13 images including
division events.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm_estimate_params/combined.pdf}
    \caption[Initial Analysis of Growth Rates in the Dataset]{
        ...
        (E) The histogram of the growth parameters exhibits significant cell-cell variability.
...
To quantify this heterogeneity, we fitted the individual curves with an exponential:
\begin{equation}
    l(t) = l_0 \exp(\mu t).
...
First, we restrict our analysis to a time interval of $\SI{20}{\minute}$ without any cell division
events, which enables us to use the computationally more efficient direct position-space metric
(Subsection~\ref{subsec:direct-metric}).
Using our complete mechanical model (see Section~\ref{sec:computational-model}), we optimized a
total of $11$ parameters: $5$ global parameters that govern mechanics and interactions (damping
...
However, to systematically analyze these differences and investigate if the obtained
parameters could be identified, we employed the \textit{profile-likelihood} method.
...
shows a steep ascent for $\lambda\rightarrow0$ which means that the observed results would be
incompatible with this choice.
Therefore, we assign a fixed value for this parameter.
...
the potential strength (E) is subject to a practical non-identifiability~\cite{Raue2009} which is
...
variety of interactions to remove this practical non-identifiability.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm_fit/morse_partial/predicted-snapshots.pdf}
    \caption[Predicted Snapshot and Visualization of the Image-Space Metric]{
        ...
        The numerically simulated results closely match the experimental data and possible
        differences cannot be seen directly.
        Therefore subfigure (B) showcases these differences by using the developed methods of
...
\begin{figure}[H]
    ...
    \caption[\textit{Likelihood Profiles} for Various Model Reduction Cases]{
...
In the next step, we extended the analyzed time interval to $\SI{85}{\minute}$, which includes cell
division events.
...
events.
This effect is frequently observed in segmentation tasks and presents a challenge in the
parameter estimation process.
One possible solution would be to manually segment the image which is often done in practice.
However, to show that the combination of our model and analysis techniques is able to deal
...
To compare experimental and simulation results via the lineage-aware image-space metric,
...
Afterwards, the corresponding mask is generated and then adjusted in such a way that it can be
...
Finally, we used the previously established lineage-aware image-based metric for the comparison.
...
required.
However, this approach is still frequently used within the optimization scheme itself and aids
...
In order to accurately calculate confidence bands for the optimized parameters, we would require
further mathematical analysis to assess how uncertainties can be incorporated into our lineage-aware
image-space metric.
Instead, we simply display profiles of the cost function directly, without normalizing by any
variances.
We fix a single parameter and reoptimize all remaining ones.
While only confidence bands for likelihood profiles as presented in the previous sections could
provide a clearer picture regarding the identifiability of the parameters, we can still
identify some trends.

We used the Morse potential as an interaction type since we have already seen in the preceding
section that the parameters of the Mie potential could not be identified properly.
...
(Figure~\ref{fig:likelihood-profiles-comparison-with-division} A - C) are well bound from below and
above.
The potential stiffness is particularly influenced by contributions from overlaps, which is well
explicable since higher parameter values result in a steeper potential curve, thus being less
permissible to overlapping radii.
Figure~\ref{fig:likelihood-profiles-comparison-with-division} (D-G) displays the growth rates of
the mother cells and (H,I) those of the two cells that do not divide, while (J-M) are the division
lengths of the mother cells (D-G).
The profiles of growth rates of the mother cells (D-G) which lead to a division event, exhibit a
stark negative peak which indicates good identifiability of these parameters.
In contrast, the profiles of growth rates (H,I) which belong to cells that do not divide during the
time interval are less pronounced.
The division lengths (J-M) also show a clear negative dip, thus enabling a proper estimation of
this parameter.
All growth rates for the daughter cells (N-P) (and (A-E) of
Figure~\ref{fig:supplement-profiles-with-division}) show similar behavior to the
non-dividing cells which are present at the initial state.
Their profiles indicate that an optimum was achieved and significantly increase with higher values
...
lengths and growth rates of cells (D-G) accurately.
In particular, it exhibits no plateaus at the minima despite the missing data points.
This phenomenon can be attributed to our mechanistic understanding of the problem and would not have
been possible by other approaches such as purely statistical models.
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm_divide/masks-diff-progression.pdf}
    \caption[Visualization of Prediction to Fit Differences]{
...
\begin{figure}[H]
    ...
    \caption[\textit{Likelihood Profiles} when including Division Events]{
        Profiles of optimized cost functions when scanning along one parameter axis.
        (A) Radius, (B) strength and (C) potential stiffness of the Morse potential.
...
bacteria.
The direct approach uses the positions of the cells which involves their extraction via our
developed algorithm.
The lineage-aware image-space metric uses only the masks of the data and thus involves one step
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c}
        ...
        \bottomrule
    \end{tabular}
    \caption[Comparison of Fitted Growth Rates and Division Lengths]{
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c}
        ...
        \bottomrule
    \end{tabular}
    \caption[Comparison of kinetic Parameters]{
...
%###################################################################################################
\section{Discussion}
It has been demonstrated that the parameters of an \ac{ib} model of flexible rod-shaped bacteria can
be estimated on the single-cell level from segmented microscopic image time series.
Profile likelihoods are indicative of the parameters that are actually constrained by the data.
In the case of the bending experiment of a single bacterium, the full mechanical model is not
identifiable; however, classical model reduction renders it identifiable.
Conversely, for a growing colony, the growth rates and division lengths of individual cells and
their descendants are recovered from routine time-lapse microscopy.
Therefore, cell-to-cell differences can be accessed without the necessity of assuming a functional
form for their distribution.
It has been demonstrated that, under certain conditions, this is applicable to any individual
parameter of the model, and not exclusively to the growth rates.
The requisite condition for this to be the case is a sufficient number of cells.

The significance of this framework is twofold.
From a methodological perspective, the comparison is intentionally agnostic to the underlying
cellular representation.
The image-space metric operates exclusively on segmentation masks and is therefore expected to be
applicable to any model—whether flexible or rigid, vertex-, spherocylinder-, or level-set-based—that
is capable of rendering labeled cell masks.
The presented methods have the potential to be reused across a wide range of rod-shaped organisms
and modeling paradigms without modification, which is a prerequisite for adoption by a broad
community.
From a biological standpoint, the estimation of parameters on a per-cell basis effectively converts
mechanical and kinetic model parameters into quantifiable values.
Rather than making assumptions about population averages, growth rates, division lengths, and
interaction strengths can be directly measured for individual cells and their lineages through the
use of routine time-lapse microscopy.
This methodology establishes a framework for quantifying cell-to-cell variability and its underlying
mechanical origins.
In doing so, it produces per-cell readouts that are not attainable through population-level
descriptions due to their inherent limitations.

Our approach differs from previous work on the calibration of \acp{abm}.
Where such models have been fitted to data, the comparison has been made at the level of population
summaries: Dancik et al.~\cite{Dancik2010} calibrate an agent-based model of \textit{Leishmania
major} infection against published macrophage and parasite counts, Lima et al.~\cite{Lima2021}
calibrate a coarse-grained two-scale tumor model against the confluence of live and dead cells
measured every three hours, and the recipes compiled by Thiele et al.~\cite{Thiele2014} fit
behavioral probabilities of an individual-based model to aggregate abundance patterns.
In each case the individual agents are simulated, but the data enter as aggregates, and the
estimated parameters characterize the population rather than its members.
In contrast, the metrics developed here operate at the level of individual cells and their lineages.
Consequently, these metrics enable the assignment of parameters to individual cells rather than to a
population average.
Furthermore, we do not merely rely on a point estimate; the profile likelihoods inform us of the
parameters that are constrained by the data.
In instances where these parameters are not constrained, the profile likelihoods suggest methods
through which the model can be reduced.
In comparison with mechanical models of rod-shaped bacteria that treat cells as rigid
spherocylinders~\cite{Rudge2012,Winkle2017}, our formulation accounts for elastic bending.
This capability is essential for the analysis of deformation experiments, such as those conducted by
Amir et al.~\cite{Amir2014,Amir2014_2}.

Several limitations should be kept in mind.
First, our benchmark of the extraction algorithm uses synthetic masks generated with the same model
that is later fitted.
Thus, it quantifies the internal consistency of the pipeline rather than its accuracy against an
independent ground truth.
Second, the two case studies are based on small datasets, three snapshots of a single bacterium and
a time series of six cells, which are sufficient to demonstrate an identifiability analysis but not
to characterize the distribution of a parameter across a population.
Third, confidence intervals were derived only for the direct position-space metric.
For the lineage-aware image-space metric, a corresponding likelihood formulation has yet to be
established.
The cost profiles reported should not be interpreted as likelihood-based confidence regions.
Finally, the estimated mechanical parameters are effective quantities of a mass-rescaled description
and are not directly comparable to independently measured material constants of the cell envelope.

Several extensions suggest themselves.
The position extraction algorithm works well for 2D cells or thin sheets, but it fails when multiple
bacteria overlap.
In its current state, the algorithm tends to underestimate the length of the rods, which leads to
skewed results.
A purpose-built machine-learning tool that can reconstruct masks or accurately extrapolate positions
from incomplete, possibly overlapping masks is well suited to tackle this task.
Currently, the parent penalty takes immediate effect at the division event and then persists
indefinitely.
Making the penalty a time-dependent quantity, $p = p(t)$, that grows with the age of the daughter
cells would allow for a much smoother transition from the pre-divided to the fully divided system.
Further biological effects, such as polar interactions~\cite{Duvernoy2018},
differentiation~\cite{vanGestel2015}, and extracellular reactions~\cite{Li2025}, can be incorporated
within the same framework.
The presented pipelines can also be used in reverse to generate near-realistic microscopic images
together with matching ground-truth masks.
These masks are needed in large numbers to train segmentation and, more importantly, cell-tracking
algorithms.
Finally, scaling the approach to larger amounts of data, most effectively by fitting several time
series simultaneously, which has yet to be implemented, would turn per-cell estimates into estimated
parameter distributions.

By choosing \texttt{cellular\_raza}~\cite{Pleyer2025} as the modeling framework, we retain
flexibility with respect to model design, since the underlying representation of the cells can be
changed as development progresses.
This is important when non-identifiability is structural, or model-based, rather than practical, or
caused by a lack of data~\cite{Heinrich2025}, because the model itself must be reformulated instead
of the dataset extended.
Overall, this work combines microscopic data directly with single-cell agent-based modeling and
establishes the foundation for treating the parameters of these models as measurable, cell-resolved
quantities.
