...
We develop a new agent-based computational model for flexible, rod-shaped bacteria which
...
shapes~\cite{Kang2014,Breitwieser2021,Gorochowski2012}, typically modeling cells as rigid
spherocylinders interacting through steric repulsion~\cite{Winkle2017}.
While such models can couple mechanical stress to cellular growth~\cite{Winkle2017}, none are able
to capture the elastic bending of flexible filamentous bacteria, nor do they provide a route to
quantitative, single-cell parameter estimation from image data.
...
Second, we develop computational methods which use segmented microscopic images to
quantitatively compare numerical results with biological data on the single-cell level.
With these methods we can estimate the parameters of our model in two case studies of increasing
complexity, each probing different aspects of our model.
...
the case of complex models such as single-cell modeling of flexible rod-shaped bacteria and when
...
Our novel mechanical model for flexible rod-shaped bacteria integrates seamlessly into our previously
...
This allows researchers to adapt the underlying cellular model to their needs, enabling
further use of the presented approach in future studies.
Overall, these contributions push for a more rigorous, interpretable, and biologically grounded
development process for \acp{abm}, which provides deeper mechanistic insights into the nature of
cellular behaviors.
%###################################################################################################
% \section{Overview}
...
Section~\ref{sec:supplement-amir-details}).
...
(D-H, bright blue curves). It can be seen that the optimized parameters are at the minimum of the
...
\begin{figure}[H]
    ...
    \caption{
        ...
        (D-H) \textit{Profile-likelihoods} for the estimated parameters.
        ...
        $\gamma=\gamma_\text{opt}$ where $\gamma_\text{opt}$ is obtained from the second (higher)
        minimum of the spring tension profile (H).
...
Figure~\ref{fig:parameter-estimates-single-step} shows the \textit{profile-likelihoods} for each
optimized parameter.
...
However, as the \textit{profile-likelihoods} presented in
Figure~\ref{fig:parameter-estimates-single-step}
...
Figure~\ref{fig:supplement-profiles-with-division} (D-I) displays the growth rates of the mother
cells while (J-M) are the division lengths, corresponding to cells (D-G).
...
The division lengths of the mother cells (D-G) also show a clear negative dip, thus enabling a
proper estimation of this parameter.
...
We also showed that unidentifiabilities can be detected and effectively reduced by classical
model reduction techniques.
We note, however, that a full identifiability analysis with confidence intervals was only carried
out for the direct position-space metric; for the lineage-aware image-space metric a corresponding
likelihood formulation remains to be established, so that the associated parameter estimates should
...
respect to computational efficiency, accuracy and extensions of the described model.
...
One significant area of improvement is the position extraction algorithm which works well for cells
in 2D or thin sheets but fails when multiple bacteria overlap.
...
event but also persists indefinitely.
This simplification should be lifted and the parent penalty should depend on the time
...
It would allow for a much smoother transition from the pre-divided to the fully-divided system.

To speed up the optimization process, it could also be valuable to initially consider all
parameters as global ones and once a certain threshold of tolerance is matched, loosening this
restriction and treating cells individually.
This would allow the algorithm to initially search within a smaller space and possibly reach the
optimum more quickly in total.
Another factor that could be considered to improve performance is that individual parameters of
cells which are spatially separated would not have an immediate effect on each other, thus presenting
a loosely coupled or fully decoupled relationship.
Depending on the optimization routine, this information could be exploited.
Ultimately, we aim to apply these methods to larger amounts of data, which is done most
effectively by fitting our model to various time-series simultaneously which is yet to be
implemented.
...
\section{Conclusion}
In this work we have laid out how to estimate the parameters of \acp{abm} for rod-shaped bacteria
on the single-cell level from microscopic image time series.
We introduced an \ac{ib} computational model that captures the elastic bending, growth, division
and mechanical interactions of flexible rods and is implemented within
\texttt{cellular\_raza}~\cite{Pleyer2025}, making it flexible in design and readily reusable.
To connect such models to experimental data, we devised methods that extract spatial positions and
further cellular information from segmented microscopic images, which are validated against our own model, 
together with a visualization pipeline that renders simulation snapshots and generates cell masks.
Building on these imaging techniques, we constructed a lineage-aware cost function that compares
cellular configurations across division events and thereby enables parameter estimation in
scenarios that were previously inaccessible.
Combining these advances, we applied \textit{profile-likelihood}-based parameter estimation to case
studies of increasing complexity and showed that the involved parameters can be identified, that
unidentifiabilities can be detected and removed by classical model reduction, and that individual
cell-to-cell differences can be recovered.

The significance of this framework lies in two directions.
Methodologically, the comparison is deliberately agnostic to the underlying cellular representation:
the image-space metric operates purely on segmentation masks and therefore applies to any model, e.g., 
flexible or rigid, vertex-, spherocylinder- or level-set-based that can render labeled cell
masks. The presented methods can thus be reused across a wide range of rod-shaped organisms and
modeling paradigms without modification, a prerequisite for adoption by a broad community.
Biologically, estimating parameters on a per-cell basis turns mechanical and kinetic model
parameters into measurable quantities: rather than assuming population averages, growth rates,
division lengths and interaction strengths can be read out for individual cells and their lineages
directly from routine time-lapse microscopy. This provides a route to quantify cell-to-cell
variability and its mechanical origins and yields testable, mechanistic readouts that purely
statistical or population-level descriptions cannot deliver.
To our knowledge, this is among the first demonstrations that single-cell parameter estimation,
including identifiability analysis, is feasible for a mechanistic individual-based model of flexible
rod-shaped bacteria.

The code is publicly available as the Python package
\href{https://github.com/jonaspleyer/cr_mech_coli}{cr\_mech\_coli}, and the underlying cellular
properties are provided within \texttt{cellular\_raza}~\cite{Pleyer2025}, which lets researchers
employ well-known methods such as \textit{profile-likelihood} and sensitivity analysis and reuse,
extend and combine existing building blocks to incorporate additional biological properties.

Future work will explore the data-generation capabilities of the software in more detail; ongoing
progress already harnesses our model and imaging pipelines to construct near-realistic microscopic
images together with corresponding cell masks, which can be used to train segmentation and, more
importantly, cell-tracking algorithms that require large amounts of data.
On the biological side, many cellular properties remain to be investigated with this technique, and
a central open question is how to scale the approach to large numbers of cells so that the
distributions of the parameters themselves can be estimated.
Taken together, this work establishes a promising platform that directly combines microscopic data
with single-cell agent-based modeling and opens a wide range of opportunities for future
quantitative studies in cellular biology.
