...
an initial state at $t=\SI{10}{\second}$, a bent state at $t=\SI{17}{\second}$ and the final
relaxed state at $t=\SI{34}{\second}$.
These acquisition times set the durations of the two simulated phases.
The bending and the subsequent elastic relaxation themselves were not resolved experimentally, so
no transient information is available for the fit.
...
The number of vertices $N_v$ should be chosen large enough such that the bending of
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crm_amir/progression.pdf}
    \caption[Comparison of Experimental Images and Computational Snapshots]{
...
\begin{table}[H]
    \begin{tabularx}{\textwidth}{l r@{} @{\hspace{0.3em}}l X}
        ...
        $N_v$ & $20$ && Number of vertices used for the discretized rod\\
        $\sigma_x$ & $0.8$&$\unit{\micro\metre}$ & Displacement uncertainty used to calculate the profile-likelihood\\
